...
async def send_request_to_service(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # print("[ydr] send_request_to_service() | request send to prefiller.")
            response.raise_for_status()
            return response
...
async def stream_service_response_with_retry(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            async with client.stream("POST", endpoint, json=req_data, headers=headers) as response:         # [ydr] 向Decoder发送POST请求
                response.raise_for_status()
...
async def _handle_select_instance(api: str, req_data: Any, request_length: int):
    ...
    request_id = await proxy_state.next_req_id()                                            # [ydr] 生成request_id
    print(f"{datetime.datetime.now()} | [ydr] _handle_select_instance() | request_id generated: {request_id}")
    ...
    print(f"{datetime.datetime.now()} | [ydr] _handle_select_instance() | request send to prefiller")
    ...
    print(f"{datetime.datetime.now()} | [ydr] _handle_select_instance() | decoder selected.")
    return InstanceInfo(
...
async def _handle_completions(api: str, request: Request):
    try:
        ...
        print(f"{datetime.datetime.now()} | [ydr] _handle_completions() | _handle_select_instance()")
        ...
        origin_max_tokens = req_data.get("max_tokens", 128)
        print(f"{datetime.datetime.now()} | [ydr] _handle_compeletions() | origin_max_tokens {origin_max_tokens}")
        ...
        async def generate_stream():
            ...
            try:
                while retry:
                    retry = False
                    print(f"{datetime.datetime.now()} | [ydr] _handle_compeletions() | stream connection with decoder.")
                    async for chunk in stream_service_response_with_retry(      # [ydr] 建立流式链接，并在其中发送POST请求
                        ...
                        if not released_kv and chunk:
                            proxy_state.release_prefiller_kv(instance_info.prefiller_idx, instance_info.prefiller_score)        # [ydr] 通知P节点释放显存
                            print(f"{datetime.datetime.now()} | [ydr] _handle_compeletions() | release prefiller kv.")
                            released_kv = True
                        try:
