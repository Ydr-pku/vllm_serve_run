...
def generate_mixed_dataset(
    ...
    p0 = p0 * 20
    ratio = generate_ratio_list(total_count, mean_percent, std_dev, num_long_prompts)
...
def generate_mixed_dataset_lognormal(
    ...
    ratio = generate_ratio_list_lognormal_with_long(total_count, mu, sigma, num_long_prompts)
    ...
    # 4. 写入 JSONL 文件 
    # 💡 提示：增加了 ensure_ascii=False，这样存出来的 jsonl 里就是中文而不是 unicode 编码
    with open(file_path, 'w', encoding='utf-8') as f:
        for p in prompts:
            f.write(json.dumps(p, ensure_ascii=False) + '\n')


def generate_ratio_list(total_count, mean_percent, std_dev, num_long_prompts):
    """
    ...
    if num_sampled > 0:
        # 使用 numpy 生成正态分布数据
        raw_ratios = np.random.normal(loc=mean, scale=std_dev, size=num_sampled)
    ...
    random.shuffle(final_ratio_list)

    return final_ratio_list

def generate_ratio_list_lognormal_with_long(total_count, mu, sigma, num_long_prompts):
    """
    ...
    if num_sampled > 0:
        # 使用对数正态分布生成
        raw_ratios = np.random.lognormal(mean=mu, sigma=sigma, size=num_sampled)
...
def generate_output_token_lengths(
    ...
    if not min_value <= mean <= max_value:
        raise ValueError("mean 必须位于 min_value 和 max_value 之间")

    rng = np.random.default_rng(seed)
    sampled_values = []

    # 使用拒绝采样实现截断正态分布，避免 np.clip 使大量样本堆积在边界。
    while len(sampled_values) < count:
        ...
        accepted = candidates[
            (candidates >= min_value) & (candidates <= max_value)
        ]
        sampled_values.extend(
            np.rint(accepted[:remaining]).astype(int).tolist()
        )
...
def gen_prompts(
    ...
    # 2. 遍历输入的 float 列表
    for ratio, output_tokens in zip(ratio_list, output_token_lengths):
        # 限制 ratio 范围
        safe_ratio = max(0.0, min(1.0, ratio))
        # 计算目标词数
        target_word_count = int(total_len * safe_ratio)
        # 截取并拼接
...
if __name__ == "__main__":
    ...
    output_mean, output_std_dev = 256, 64
    output_min, output_max, output_seed = 64, 512, 42
    
    generate_mixed_dataset_lognormal(
    ...
    print(
        ...
        f" | output_tokens~TruncNormal(mean={output_mean}, "
        f"std={output_std_dev}, min={output_min}, max={output_max}, "
        f"seed={output_seed})"
    )
